"""[CO-09] API 单测。TestClient 不可用时自动 skip。"""
...
if _AVAILABLE:
    try:
        from modules.co_09.main import app
    except (ImportError, TypeError):
        _AVAILABLE = False
...
class TestAPI(unittest.TestCase):
    ...
    def test_health(self):
        ...
        self.assertEqual(resp.json()["module"], "CO-09")
    ...
    def test_execute_returns_ok(self):
        resp = self.client.post("/api/v1/execute", json={"policies": []})
        self.assertEqual(resp.status_code, 200)
        data = resp.json()
    ...
    def test_execute_with_data(self):
        resp = self.client.post("/api/v1/execute", json={
            "policies": [{"policy_id": "T1", "text": "隐私政策测试"}]
        })
        self.assertEqual(resp.status_code, 200)
