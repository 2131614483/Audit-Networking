"""[FA-08] API 单测（unittest 风格，TestClient 懒加载避免环境兼容问题）。"""
...
class TestAPI(unittest.TestCase):
    ...
    def setUp(self):
        try:
            ...
            from modules.fa_08.main import app
    ...
    def test_health(self):
        """健康检查返回 module=FA-08, status=ok。"""
        r = self.client.get("/api/v1/health")
        self.assertEqual(r.status_code, 200)
        body = r.json()
        self.assertEqual(body["module"], "FA-08")
    ...
    def test_info(self):
        ...
        body = r.json()
        self.assertEqual(body["module"], "FA-08")
        self.assertEqual(body["name"], "底稿自动勾稽检查")
