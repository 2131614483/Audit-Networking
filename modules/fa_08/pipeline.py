"""[FA-08] 执行管道 —— 采集 → 处理 → 输出三阶段。

编排顺序：
  collect(接入数据归一化) → engine.execute(预处理→推理→后处理)
  → apply_thresholds(严重度升级/置信度分级) → apply_custom_rules(业务规则)
...
class Pipeline:
    ...
    def __init__(self, config: dict | None = None):
        ...
        # 显式触发模型加载：初始化 CONSISTENCY_RULES
        self.engine.setup()
    ...
    def _collect(self, input_data: Any) -> Any:
        ...
        out = {
            ...
            "statements": input_data.get("statements", input_data.get("报表", {})),
            "vouchers": input_data.get("vouchers", input_data.get("凭证", [])),
            "metrics": input_data.get("metrics", input_data.get("指标", [])),
        }
        # 透传 context 等额外字段
        for k, v in input_data.items():
            if k not in ("workpapers", "底稿", "statements", "报表",
                         "vouchers", "凭证", "metrics", "指标"):
                out.setdefault(k, v)
        return out
